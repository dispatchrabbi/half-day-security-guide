...
function reindexFootnoteListing(footnotesDiv, footnoteMap) {
  ...
  footnotes.forEach(function(footnote) {
    ...
    if(!remappedIndex) { throw new Error(`No footnote remapping found for ${formerlyKnownAs}! Something hinky is happening.`)};
...
function reindexFootnotes(bodyHast) {
  const footnotesDiv = bodyHast.children.filter(child => child.tagName === 'div' && child.properties.className.includes('footnotes'))[0];
  if(!footnotesDiv) { throw new Error(`No footnotes div found. Did you call unified.use(footnotes)?`)};
